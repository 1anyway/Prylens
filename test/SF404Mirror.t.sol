...
import {MockSF404Ownable} from "./utils/mocks/MockSF404Ownable.sol";
import {SF404Mirror, MockSF404Mirror} from "./utils/mocks/MockSF404Mirror.sol";
import {LibSort} from "solady/utils/LibSort.sol";

contract Invalid721Receiver {}

contract SF404MirrorTest is SoladyTest {
    ...
    MockSF404 dn;
    MockSF404Mirror mirror;

    function setUp() public {
        dn = new MockSF404();
        mirror = new MockSF404Mirror(address(this));
    }

    function testNotLinked() public {
        vm.expectRevert(SF404Mirror.NotLinked.selector);
        mirror.name();
    }
    ...
    function testSetAndGetApproved() public {
        dn.initializeSF404(uint96(10 * _WAD), address(this), address(mirror));
        ...
        vm.expectRevert(SF404.TransferCallerNotOwnerNorApproved.selector);
        vm.prank(bob);
    ...
    function testTransferFrom(uint32 totalNFTSupply) public {
        ...
        dn.initializeSF404(initialSupply, address(this), address(mirror));
    ...
    function testTransferFromMixed(uint256) public {
        ...
        dn.initializeSF404(maxNFTId * _WAD, address(this), address(mirror));
    ...
    function testSafeTransferFrom(uint32 totalNFTSupply) public {
        ...
        dn.initializeSF404(uint96(uint256(totalNFTSupply) * _WAD), address(this), address(mirror));
        dn.transfer(alice, _WAD * uint256(5));
        assertEq(mirror.balanceOf(alice), 5);
        assertEq(mirror.balanceOf(bob), 0);

        vm.prank(alice);
        vm.expectRevert(SF404Mirror.TransferToNonERC721ReceiverImplementer.selector);
    ...
    function testLinkMirrorContract() public {
        ...
        assertEq(data, abi.encodePacked(SF404Mirror.SenderNotDeployer.selector));
    ...
    function testLogDirectTransfers() public {
        dn.initializeSF404(5 * _WAD, address(this), address(mirror));
    ...
    function testPullOwnerWithOwnable() public {
        MockSF404Ownable dnOwnable = new MockSF404Ownable();
        dnOwnable.initializeSF404(1000, address(this), address(mirror));
    ...
    function testFnSelectorNotRecognized() public {
        ...
        assertFalse(success);
        assertEq(result, abi.encodePacked(SF404.FnSelectorNotRecognized.selector));
    }
}
